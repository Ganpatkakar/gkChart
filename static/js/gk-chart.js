...
function PrintContent(event, canvasWidth, canvasHeight, chartID) {
  try {
    ...
    console.log("Error in PrintContent Function", error);
  }
}
    ...
    try {
      if (chart.config.printEnable != undefined && chart.config.printEnable == true) {
        ...
        document.querySelector("#" + chartID + " #png_" + chartID).addEventListener('click', function (event) {
          //Canvas2Image.saveAsPNG(document.getElementById(`canvas${chartID}`), document.getElementById(`canvas${chartID}`).width, document.getElementById(`canvas${chartID}`).height);
          css(document.querySelector(`#${chartID} #print-options`), {
            "display": "none"
          });
          visible = false;
          ...
          let nimage = image.replace("image/png", "image/octet-stream");
          document.location.href = nimage;
        });
      }
    } catch (error) {
      console.log(error);
