...
function () {
  ...
  _createClass(ChartSurface, [{
    key: "ratio",
    value: function ratio(canvasContainer) {
      var ctx = canvasContainer.getContext('2d');
      var dpr = window.devicePixelRatio || 1;
      var bsr = ctx.webkitBackingStorePixelRatio || ctx.mozBackingStorePixelRatio || ctx.msBackingStorePixelRatio || ctx.oBackingStorePixelRatio || ctx.backingStorePixelRatio || 1; // return dpr / bsr;
    ...
    value: function prepSurface(nr, width, height, container) {
      try {
        ...
        canvasDom.style.height = height + "px"; // document.getElementById('container').append('<canvas id="canvas' + nr + '" class="canvas"' +
    ...
    value: function prepUI(nr) {
      try {
        ...
        ctx.font = "23px arial";
        ctx.lineWidth = 1; // console.log("End : prepUI");
    ...
    value: function preparePlot(nr, sizex, sizey, container) {
      try {
        ...
        var canvasContext = this.prepUI(nr); // console.log("End : preparePlot");
    ...
    value: function prepSurfaceupper(nr, width, height, container) {
      try {
        ...
        canvasDom.height = height * this.ratio(canvasDom);
        canvasDom.style.width = width + "px";
        canvasDom.style.height = height + "px"; // console.log("End : prepSurfaceupper");
      } catch (e) {
    ...
    value: function prepUIUpper(nr) {
      try {
        ...
        var ctx = canvas.getContext('2d'); //ctx.font = '18px Arial';

        ctx.lineWidth = 1; // console.log("End : prepUIUpper");
    ...
    value: function preparePlotUpper(nr, sizex, sizey, container) {
      try {
        ...
        var canvasContext = this.prepUIUpper(nr); // console.log("End : preparePlotUpper");
...
function drawGrid(nr, verticanNr, ctx, data) {
  try {
    ...
    var canvas = document.getElementById('canvas' + nr);
    var hei = canvas.height - 60; //// console.log("canvas height to draw grid lines:" + hei);

    var wid = canvas.width - 100; //// console.log("canvas width to draw grid lines:" + wid);

    ctx.beginPath();
    ctx.fillStyle = "#000";
    var spacingVertical = hei / verticanNr; //// console.log("canvas vertical spacings to draw grid lines:" + spacingVertical);

    var spacingHorizontal = wid / data[0].datapoints.length; //// console.log("canvas horizontal spacings to draw grid lines:" + spacingHorizontal);
    ...
    ctx.stroke(); // vartical other grid rows
...
function drawGraphicLinearYcord(canvas, ctx, verticalNr, cdata) {
  try {
    ...
    var spacingHorizontal = wid / cdata.data[0].datapoints.length; //// console.log(spacingHorizontal);
    ...
    if (xangle === 'angular') {
      for (var i = 0; i < cdata.data[0].datapoints.length; i++) {
        ...
        ctx.fillText(cdata.data[0].datapoints[i].label, 0, 0); //// console.log(cdata.xaxis.categories[i], i*spacingHorizontal, hei-spacingVertical);
      ...
      for (var i = 0; i < cdata.data[0].datapoints.length; i++) {
        var textWidth = ctx.measureText(cdata.data[0].datapoints[i].label).width;
        var fromLeft = i * spacingHorizontal + spacingHorizontal / 2 + 100 - textWidth / 2;
    ...
    ctx.closePath(); // console.log("End : drawGraphicLinearYcord");
  } catch (e) {
...
function () {
  ...
  _createClass(DrawChart, [{
    key: "drawGraphicLinear",
    value: function drawGraphicLinear(canvas, ctx, verticalNr, data, range, chartColor, linecord) {
      try {
        var calcWaypoints = function calcWaypoints(vertices) {
          var waypoints = [];
        ...
        var animate = function animate() {
          ...
          ctx.stroke(); // increment "t" to get the next waypoint
        ...
        var commonCodeCircle = function commonCodeCircle() {
          ctx.beginPath();
        ...
        var fillAreaCall = function fillAreaCall(vertices) {
          ctx.beginPath();
          ...
          for (var _points = 0; _points < vertices.length; _points++) {
            ctx.lineTo(vertices[_points].x, vertices[_points].y + 4);
          }
        ...
        var hei = canvas.height - 60;
        var wid = canvas.width - 100;
        var spacingVertical = hei / verticalNr; //// console.log("spacingVertical:" + spacingVertical);

        var spacingHorizontal = wid / data.datapoints.length; //// console.log("spacingHorizontal:" + spacingHorizontal);
        ...
        animate();
        ctx.globalAlpha = 1; // console.log("End : drawGraphicLinear");
    ...
    key: "drawStepchart",
    value: function drawStepchart(canvas, ctx, verticalNr, data, range, chartColor, linecord) {
      try {
        var calcWaypoints = function calcWaypoints(vertices) {
          var waypoints = [];
        ...
        var fillAreaCall = function fillAreaCall(vertices) {
          ...
          for (var _points2 = 0; _points2 < vertices.length; _points2++) {
            ctx.lineTo(vertices[_points2].x, vertices[_points2].y + 4);
          }
        ...
        points.push(localLineCords[localLineCords.length - 1]);
        animate();
        ctx.globalAlpha = 1; // console.log("End : drawStepchart");
    ...
    key: "drawBar",
    value: function drawBar(canvas, ctx, verticalNr, data, range, curx, chartColor, linecord, barwidth, barChartCount) {
      try {
        var calcWaypoints = function calcWaypoints(points) {
          var wayPoints = [];

          for (var _i = 0; _i < points.length; _i++) {
            var x1 = points[_i].x;
            var totalHeight = hei;
            var _rectHeight = points[_i].hei;
            var currentHeight = 0; //// console.log(rectHeight);

            var newWayPoint = [];

            while (currentHeight <= _rectHeight) {
              newWayPoint.push({
                x: x1,
              ...
              var difference = _rectHeight - currentHeight;
        ...
        var animate = function animate(animateArr, t, cColor) {
          ctx.beginPath();
          ctx.globalAlpha = 1;
          ctx.fillStyle = cColor;
          ctx.fillRect(animateArr[t].x, animateArr[t].y, animateArr[t].wid, animateArr[t].hei); //ctx.stroke();
        ...
        for (var i = 0; i < data.datapoints.length; i++) {
          ...
          var barChartWidth = barChartCount * barwidth + (barChartCount - 1) * 5;
          var fromLeft = i * spacingHorizontal + spacingHorizontal / 2 + curx - barChartWidth / 2;
          var newobj = {
            x: fromLeft,
            y: rectHeight,
            wid: barwidth,
        ...
        ctx.closePath();
        var points = calcWaypoints(localLinecord);

        for (var _i2 = 0; _i2 < points.length; _i2++) {
          var t = 0;
          var cColor = chartColor;
          animate(points[_i2], t, cColor);
        }
    ...
    key: "drawPie",
    value: function drawPie(canvas, ctx, data, linecord) {
      try {
        ...
        for (var i = 0; i < data.datapoints.length; i++) {
          ...
          if (data.datapoints[i].y == 0) {
            ctx.arc(canvas.width / 2, canvas.height / 2, canvas.height / 2, lastend, lastend + Math.PI * 2 * (data.datapoints[i].y / myTotal), false); //// console.log(canvas.width / 2, canvas.height / 2, canvas.height / 2, lastend, lastend + (Math.PI * 2 * (data.datapoints[i].y / myTotal)));
          ...
          linecord.push(newobj);
          lastend += Math.PI * 2 * (data.datapoints[i].y / myTotal); //// console.log(lastend);
        ...
        for (i = 0; i < data.datapoints.length; i++) {
          if (data.datapoints[i].y != 0) {
            ...
            var fy = radius + radius * .7 * Math.sin(angle + anglemiddle); //ctx.moveTo(x, y);

            ctx.translate(fx, fy); //ctx.rotate(angle + anglemiddle);
    ...
    key: "drawDonut",
    value: function drawDonut(canvas, ctx, data, linecord, chartHeight) {
      try {
        ...
        for (var i = 0; i < data.datapoints.length; i++) {
          ...
          ctx.arc(canvas.width / 2, canvas.height / 2, radius, lastend, lastend + Math.PI * 2 * (data.datapoints[i].y / myTotal), false); //// console.log(canvas.width / 2, canvas.height / 2, canvas.height / 2, lastend, lastend + (Math.PI * 2 * (data.datapoints[i].y / myTotal)));
          //ctx.lineTo(canvas.width / 2, canvas.height / 2);
        ...
        for (i = 0; i < data.datapoints.length; i++) {
          if (data.datapoints[i].y != 0) {
            ...
            var fy = radius * 1.5 + radius * Math.sin(angle + anglemiddle); //ctx.moveTo(x, y);
    ...
    key: "drawMeter",
    value: function drawMeter(canvas, ctx, verticalNr, data, range, chartColor, ChartDataToShow) {
      try {
        ...
        for (var i = 0; i < data.datapoints.length; i++) {
          ...
          ctx.beginPath();
          ctx.moveTo(canvas.width / 2, canvas.height / 2);
          ctx.arc(canvas.width / 2, canvas.height / 2, radius, lastend, lastend + Math.PI * (data.datapoints[i].y / myTotal)); //// console.log(canvas.width / 2, canvas.height / 2, canvas.height / 2, lastend, lastend + (Math.PI * 2 * (data.datapoints[i].y / myTotal)));
        ...
        ctx.closePath();
        ctx.beginPath();
        var rotateangel = Math.PI * (ChartDataToShow / 100) + 3.141592653589793; //// console.log(rotateangel);
        ...
        ctx.restore(); // console.log("End : drawMeter");
    ...
    key: "drawsplinechart",
    value: function drawsplinechart(canvas, ctx, verticalNr, data, range, chartColor, linecord) {
      try {
        var calcWaypoints = function calcWaypoints(vertices) {
          var waypoints = [];
          ...
          for (var i = 1; i < vertices.length; i += 3) {
            ...
            for (var t = 0; t < difference; t++) {
              var pointers = getQuadraticBezierXYatT(startPt, ct1, ct2, endPt, t / difference);
              waypoints.push({
                x: pointers.x,
                y: pointers.y
              });
        ...
        var getQuadraticBezierXYatT = function getQuadraticBezierXYatT(startPt, ct1, ct2, endPt, t) {
          var x = Math.pow(1 - t, 3) * startPt.x + 3 * Math.pow(1 - t, 2) * t * ct1.x + 3 * (1 - t) * Math.pow(t, 2) * ct2.x + Math.pow(t, 3) * endPt.x;
          var y = Math.pow(1 - t, 3) * startPt.y + 3 * Math.pow(1 - t, 2) * t * ct1.y + 3 * (1 - t) * Math.pow(t, 2) * ct2.y + Math.pow(t, 3) * endPt.y;
          return {
            x: x,
            y: y
          };
        ...
        var animate = function animate(points) {
        ...
        var fillAreaCall = function fillAreaCall(vertices) {
          ...
          for (var _points3 = 0; _points3 < vertices.length; _points3++) {
            ctx.lineTo(vertices[_points3].x, vertices[_points3].y + 4);
          }
        ...
        for (var _i3 = 0; _i3 < data.datapoints.length; _i3++) {
          var newobj = {
            x: _i3 * spacingHorizontal + spacingHorizontal / 2 + 100,
            y: hei - (data.datapoints[_i3].y - range[0]) * verticalCoefficient,
            label: data.datapoints[_i3].label,
            dataLabel: data.dataLabel,
            dataval: data.datapoints[_i3].y,
            dataColor: data.chartColor
          }; //// console.log(newobj);
        ...
        ctx.closePath();
        var f = 2;
        var a = this.bezierPointsCalc(localLineCords, f); // console.log(a);
        ...
        points.push(localLineCords[localLineCords.length - 1]);
        var i = 0; //ctx.beginPath();

        animate(points);
        ctx.globalAlpha = 1; // if (data.fill) {
        //   ctx.beginPath();
...
function () {
  ...
  _createClass(DrawChartUpperCanvas, [{
    key: "ratio",
    value: function ratio(ctx) {
      var dpr = window.devicePixelRatio || 1;
      var bsr = ctx.webkitBackingStorePixelRatio || ctx.mozBackingStorePixelRatio || ctx.msBackingStorePixelRatio || ctx.oBackingStorePixelRatio || ctx.backingStorePixelRatio || 1;
      return 2; //return dpr / bsr;
    }
  }, {
    key: "lineChartUpperCanvas",
    value: function lineChartUpperCanvas(nr, ctx, linecord, container, chart) {
      try {
        // console.log("Start : lineChartUpperCanvas");
        var dataPointLen = chart.data[0].datapoints.length;
        var wid = document.getElementById('canvasupper' + nr).width - 100;
        var spacingHorizontal = wid / dataPointLen;
        var lineCordRepeat = linecord.length / dataPointLen; // console.log(spacingHorizontal);
        ...
        document.getElementById('canvasupper' + nr).addEventListener('mousemove', function (evt) {
          ...
          for (var i = 0; i < dataPointLen; i++) {
            ctx.beginPath();
            var x1 = i * spacingHorizontal + 100;
            var x2 = spacingHorizontal;
            var y1 = 0;
            var y2 = document.getElementById('canvasupper' + nr).height - 60;
            ctx.rect(x1, y1, x2, y2);

            if (ctx.isPointInStroke(mousePos.x, mousePos.y) || ctx.isPointInPath(mousePos.x, mousePos.y)) {
              //ctx.clearRect(0, 0, document.getElementById('canvasupper' + nr).width, document.getElementById('canvasupper' + nr).height);
              for (var j = 0; j < lineCordRepeat; j++) {
                ctx.beginPath();
                var position = j * dataPointLen + i;
                ctx.arc(linecord[position].x, linecord[position].y, 12, 0, 2 * Math.PI); // console.log("lineChart compare mouse over on upper canvas");
                //ctx.lineWidth = 5;
                ...
                this.details += "<div style=\"color: ".concat(linecord[position].dataColor, "\">").concat(linecord[position].dataLabel, "  <br />  ").concat(linecord[position].label, " : ").concat(linecord[position].dataval, " <br /></div>");
              }

              ctx.beginPath();
              var lineX = x1 + spacingHorizontal / 2;
        ...
        }.bind(this), false); // console.log("End : lineChartUpperCanvas");
      } catch (e) {
    ...
    value: function barChartUpperCanvas(nr, ctx, linecord, container, chart) {
      try {
        // console.log("Start : barChartUpperCanvas");
        var dataPointLen = chart.data[0].datapoints.length;
        var wid = document.getElementById('canvasupper' + nr).width - 100;
        var spacingHorizontal = wid / dataPointLen;
        var lineCordRepeat = linecord.length / dataPointLen;
        document.getElementById('canvasupper' + nr).addEventListener('mousemove', function (evt) {
          var mousePos = getMousePos(document.getElementById('canvasupper' + nr), evt); //// console.log(mousePos);
        ...
        }.bind(this), false); // console.log("End : barChartUpperCanvas");
      } catch (e) {
    ...
    key: "clearDetails",
    value: function clearDetails(nr, ctx, container) {
      document.getElementById('canvasupper' + nr).addEventListener('mousemove', function (evt) {
        this.details = '';
        ctx.clearRect(0, 0, document.getElementById('canvasupper' + nr).width, document.getElementById('canvasupper' + nr).height);
      }.bind(this));
    ...
    value: function pieChartUpperCanvas(nr, ctx, linecord, container) {
      try {
    ...
    value: function donutChartUpperCanvas(nr, ctx, linecord, container) {
      try {
        ...
        document.getElementById('canvasupper' + nr).addEventListener('mousemove', function (evt) {
          ...
          for (var i = 0; i < linecord.length; i++) {
            ...
            ctx.lineWidth = linewidth * 2;
            ctx.beginPath(); //ctx.lineTo(linecord[i].x, linecord[i].x);

            ctx.arc(linecord[i].wid / 2, linecord[i].hei / 2, radius, linecord[i].startangle, linecord[i].lastangle, false); //ctx.lineTo(linecord[i].x, linecord[i].x);
...
function writeMessage(canvas, message) {
  try {
    ...
    //ctx.font = '18px Arial';
    ctx.fillStyle = 'black';
    ctx.fillText(message, 10, 25); // console.log("End : writeMessage");
  } catch (e) {
  ...
  _createClass(chartCalling, [{
    ...
    value: function lineChart(chart, chartID) {
      try {
        ...
        chart.hei = ChartContainer.clientHeight - 33; //// console.log("chart.wid :" + chart.wid + ", chart.hei : " + chart.hei);
        ...
        var ctx_base = this.chartSurface.preparePlot(chart.chartnumber, chart.wid, chart.hei, chart.container);
        ...
        var verticaldevisions = Math.floor((chart.yaxis.max - chart.yaxis.min) / chart.yaxis.difference); //// console.log("verticaldevisions" + verticaldevisions);
        ...
        for (var _i4 = 0; _i4 < chart.data.length; _i4++) {
          this.drawChart.drawGraphicLinear(canvas, ctx_base, verticaldevisions, chart.data[_i4], maxdata, chart.data[_i4].chartColor, linecord);
        }

        drawGraphicLinearYcord(canvas, ctx_base, verticaldevisions, chart); // console.log(linecord);

        var ctx_upper = this.chartSurface.preparePlotUpper(chart.chartnumber, chart.wid, chart.hei, chart.container);
    ...
    value: function barChart(chart, chartID) {
      try {
        ...
        var barwidth = drawGrid(chart.chartnumber, verticaldevisions, ctx_base, chart.data); //// console.log("barwidth:" + barwidth);

        var canvas = 'canvas' + chart.chartnumber;
        var rangedata = [chart.yaxis.min, chart.yaxis.max]; //// console.log("maxdata:" + maxdata);
        ...
        for (var i = 0; i < chart.data.length; i++) {
          ...
          nextcurve += barwidth + 5; //this.drawChart.drawGraphicLinear(canvas, ctx_base, verticaldevisions, chart.data[i], maxdata, chart.data[i].chartColor, linecord);
        }
        ...
        var ctx_upper = this.chartSurface.preparePlotUpper(chart.chartnumber, chart.wid, chart.hei, chart.container);
        this.drawUpperChart.clearDetails(chart.chartnumber, ctx_upper, chart.container);
        this.drawUpperChart.barChartUpperCanvas(chart.chartnumber, ctx_upper, linecord, chart.container, chart); // console.log("End : barChart");
    ...
    value: function pieChart(chart, chartID) {
      try {
        ...
        titleAndPrintButton += this.printOptions(chartID, chart);
        ChartContainer.innerHTML = titleAndPrintButton;
        var ctx_base = this.chartSurface.preparePlot(chart.chartnumber, chart.wid, chartHeight, chart.container); //drawGrid(chart.chartnumber, 10, ctx_base, chart.data);

        var canvas = 'canvas' + chart.chartnumber; // -- Removed the Calculation of maxdata, since it is not used for pie chart 1 Feb 2019 -- //
        // let maxdata = [];
        ...
        var ctx_upper = this.chartSurface.preparePlotUpper(chart.chartnumber, chart.wid, chartHeight, chart.container); // Removed unused parameters 1 Feb 2019
        ...
        for (var i = 0; i < chart.data[0].datapoints.length; i++) {
          pieChartDataDisplay += '<li style="width: 50%; float: left">' + '<span style="width:20px; height: 10px; display: inline-block; margin-right: 10px; background-color:' + chart.data[0].datapoints[i].color + '; border: 1px solid black; border-radius: 2px;"></span>' + '<span>' + chart.data[0].datapoints[i].label + ' : ' + chart.data[0].datapoints[i].y + '</span>' + '</li>';
        }

        pieChartDataDisplay += '</ul>';
        chartContainerSelector.insertAdjacentHTML('beforeend', pieChartDataDisplay);
        this.printAction(chartID, chart); // console.log("End : pieChart");
      } catch (err) {
    ...
    value: function donutChart(chart, chartID) {
      try {
        ...
        titleAndPrintButton += this.printOptions(chartID, chart);
        ChartContainer.innerHTML = titleAndPrintButton;
        var ctx_base = this.chartSurface.preparePlot(chart.chartnumber, chart.wid, chartHeight, chart.container);
        ...
        var ctx_upper = this.chartSurface.preparePlotUpper(chart.chartnumber, chart.wid, chartHeight, chart.container);
        this.drawUpperChart.donutChartUpperCanvas(chart.chartnumber, ctx_upper, linecord, chart.container);
        var pieChartDataDisplay = '<ul style="list-style: none; width: ' + chart.wid + 'px; padding: 0px; display: inline-block; position: relative; top: ' + chart.hei + 'px">';

        for (var _i5 = 0; _i5 < chart.data[0].datapoints.length; _i5++) {
          pieChartDataDisplay += '<li style="width: 50%; float: left">' + '<span style="width:20px; height: 10px; display: inline-block; margin-right: 10px; background-color:' + chart.data[0].datapoints[_i5].color + '; border: 1px solid black; border-radius: 2px;"></span>' + '<span>' + chart.data[0].datapoints[_i5].label + ' : ' + chart.data[0].datapoints[_i5].y + '</span>' + '</li>';
        }

        pieChartDataDisplay += '</ul>';
        chartContainerSelector.insertAdjacentHTML('beforeend', pieChartDataDisplay);
        this.printAction(chartID, chart); // console.log("End : donutChart");
      } catch (err) {
    ...
    value: function meterChart(chart, chartID) {
      try {
        ...
        var linewidth = 50;
        ChartDataToShow = Math.round(ChartDataToShow / meterTotal * 100); //// console.log(ChartDataToShow);

        this.drawChart.drawMeter(canvas, ctx_base, 10, chart.data[0], maxdata, chart.data[0].chartColor, ChartDataToShow);
        this.printAction(chartID, chart); // console.log("End : meterChart");
      } catch (err) {
    ...
    value: function column(chart, chartID) {
      try {
        ...
        var canvas = 'canvas' + chart.chartnumber;
        var maxdata = [chart.yaxis.min, chart.yaxis.max]; //// console.log("maxdata:" + maxdata);
        ...
        for (var _i6 in chart.data) {
          chart.data[_i6].type == "bar" ? barChartCount++ : null;
        }

        for (var _i7 = 0; _i7 < chart.data.length; _i7++) {
          if (chart.data[_i7].type == "bar") {
            this.drawChart.drawBar(canvas, ctx_base, verticaldevisions, chart.data[_i7], maxdata, nextcurve, chart.data[_i7].chartColor, barCords, barwidth, barChartCount);
            nextcurve += barwidth + 5;
          }

          if (chart.data[_i7].type == "line") {
            this.drawChart.drawGraphicLinear(canvas, ctx_base, verticaldevisions, chart.data[_i7], maxdata, chart.data[_i7].chartColor, lineLineCords);
          }

          if (chart.data[_i7].type == "spline") {
            var splineCord = this.drawChart.drawsplinechart(canvas, ctx_base, verticaldevisions, chart.data[_i7], maxdata, chart.data[_i7].chartColor, lineLineCords);
          }
        }

        drawGraphicLinearYcord(canvas, ctx_base, verticaldevisions, chart); // console.log("barCords ", barCords);
        ...
        this.printAction(chartID, chart); // console.log("End : barChart");
      } catch (err) {
        console.error("Exception occurred in bar chart module:  " + err.message);
      }
    }
    ...
    value: function splineChart(chart, chartID) {
      try {
        ...
        chart.chartnumber = chartID;
        var ChartContainer = document.querySelector("#" + chart.container);
        chart.wid = ChartContainer.clientWidth - 10; //chart.wid = chart.width;
        ...
        for (var _i8 = 0; _i8 < chart.data.length; _i8++) {
          this.drawChart.drawsplinechart(canvas, ctx_base, verticaldevisions, chart.data[_i8], maxdata, chart.data[_i8].chartColor, linecord);
        } //// console.log(linecord);
        ...
        this.drawUpperChart.clearDetails(chart.chartnumber, ctx_upper, chart.container);
        this.drawUpperChart.lineChartUpperCanvas(chart.chartnumber, ctx_upper, linecord, chart.container, chart);
        this.printAction(chartID, chart); // console.log("End : splineChart");
      } catch (err) {
        console.error("Exception occurred in line chart module:  " + err.message);
      }
    }
    ...
    value: function stepChart(chart, chartID) {
      try {
        ...
        for (var _i9 = 0; _i9 < chart.data.length; _i9++) {
          this.drawChart.drawStepchart(canvas, ctx_base, verticaldevisions, chart.data[_i9], maxdata, chart.data[_i9].chartColor, linecord);
        } // console.log(linecord);


        var ctx_upper = this.chartSurface.preparePlotUpper(chart.chartnumber, chart.wid, chart.hei, chart.container);
        this.drawUpperChart.clearDetails(chart.chartnumber, ctx_upper, chart.container);
        this.drawUpperChart.lineChartUpperCanvas(chart.chartnumber, ctx_upper, linecord, chart.container, chart); // this.drawUpperChart.lineChartUpperCanvas(chart.chartnumber, ctx_upper, chart.wid, chart.hei, linecord, chart.container, chart.type);

        this.printAction(chartID, chart); // console.log("End : stepChart");
      } catch (error) {
    ...
    key: "printOptions",
    value: function printOptions(chartID, chart) {
      try {
        if (chart.config.printEnable != undefined && chart.config.printEnable == true) {
          var content = "\n                        <div id=\"print-option-menu\" style=\"\n                                        position: absolute;\n                                        right: 25px;\n                                        margin-top:-65px;\n                                        border-radius:4px;\n                                        border:1px solid #000;\n                                        background: rgba(0,0,0,.3);\n                                        cursor:pointer;\">\n                          <div style=\"width:30px\">\n                            <hr style=\"margin: 5px;\" />\n                          </div>\n                          <div style=\"width:30px\">\n                            <hr style=\"margin: 5px;\" />\n                          </div>\n                          <div style=\"width:30px\">\n                            <hr style=\"margin: 5px;\" />\n                          </div>\n                        </div>\n                        <div id=\"print-options\" style=\"display:none;\">\n                          <div style=\" padding: 3px 5px;cursor: pointer;\" id=\"print_".concat(chartID, "\">Print</div>\n                          <div style=\" padding: 3px 5px;cursor: pointer;\" id=\"jpg_").concat(chartID, "\">Save as JPEG</div>\n                          <div style=\" padding: 3px 5px;cursor: pointer;\" id=\"png_").concat(chartID, "\">Save as PNG</div>\n                        </div>\n                        ");
          return content;
        }
    ...
    key: "printAction",
    value: function printAction(chartID, chart) {
      try {
        if (chart.config.printEnable != undefined && chart.config.printEnable == true) {
          var visible = false;
          document.querySelector("#".concat(chartID, " #print-option-menu")).addEventListener('click', function (event) {
            css(document.querySelector("#".concat(chartID, " #print-options")), {
              "position": "absolute",
              "right": "25px",
              "border": "1px solid #000",
          ...
          document.querySelector("#" + chartID + " #jpg_" + chartID).addEventListener('click', function (event) {
            Canvas2Image.saveAsImage(document.getElementById("canvas".concat(chartID)), document.getElementById("canvas".concat(chartID)).width, document.getElementById("canvas".concat(chartID)).height);
            css(document.querySelector("#".concat(chartID, " #print-options")), {
              "display": "none"
            });
            visible = false;
...
function () {
  ...
  function GKChart(data) {
    ...
    try {
      ...
      this.initialize.bind(this)(); // console.info("Exit: Chart Designing initialize function");
    } catch (err) {
  ...
  _createClass(GKChart, [{
    ...
    value: function initialize() {
      try {
        ...
        }.bind(this)); // console.log("End : initialize");
      } catch (err) {
